...
export default function CartPage() {
  ...
  const updateMerchandiseItem = useCartStore(
    (state) => state.updateMerchandiseItem,
  );
  ...
  useEffect(() => {
    ...
    window.history.pushState(
      { ...window.history.state, merchCartSummary: true },
      "",
    );
  ...
  const legacyShipping = calculateDeliveryShipping(items);
  const subtotal = getTotal();
  const legacyGrandTotal = subtotal + legacyShipping;
    ...
    return (
      <div className="container" style={{ paddingTop: 48, paddingBottom: 48 }}>
        <div style={{ marginBottom: 24 }}>
          <Link href="/" style={{ color: "var(--tw-blue)", fontWeight: 500 }}>
            ← Continue Shopping
          </Link>
        </div>
        ...
        {items.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 20px" }}>
            ...
            <p style={{ color: "var(--text-muted)", marginBottom: 24 }}>
              Browse our SIM plans and add items to get started.
            </p>
          ...
          <div className="cart-grid">
              ...
              {items.map((item) => (
                <div key={item.id} className="cart-item-card">
                  ...
                  <div className="cart-item-info">
                    ...
                    {item.simType && (
                      <span className="sim-type-badge">
                        {item.simType === "esim" ? "eSIM" : "Physical SIM"}
                      </span>
                    )}
            ...
            <div className="cart-summary">
              ...
              <div className="cart-summary-row">
                <span>Shipping</span>
                <span>
                  {legacyShipping === 0 ? "FREE" : formatRM(legacyShipping)}
                </span>
              </div>
              <p
                style={{
              ...
              >
                Merchandise delivery is RM10 per order. SIM shipping rules
                remain unchanged. Self-pickup is free.
              </p>
              <div className="cart-summary-total">
                <span>Total</span>
                <span>{formatRM(legacyGrandTotal)}</span>
              </div>
              <button
      ...
      <div className="container merch-cart-container">
        ...
        {items.length === 0 ? (
          <section className="merch-cart-empty">
            ...
            <Link href={merchandiseLink} className="btn btn-primary">
              Continue Shopping
            </Link>
          </section>
        ) : (
            ...
            <div className="merch-cart-layout">
                    ...
                    <div className="merch-cart-item-copy">
                      ...
                      <div className="merch-cart-config-row">
                        {item.type === "merchandise" ? (
                          <button
                            type="button"
                            className="merch-cart-variation"
                            onClick={() => setEditingItem(item)}
                          >
                            <span>
                              {[
                                item.variant || item.selectionRequired,
                                item.size,
                              ]
                                .filter(Boolean)
                                .join(" | ")}
              ...
              <aside className="merch-cart-summary">
                ...
                <div className="merch-cart-shipping-note">
                  ...
                  <span>
                    Courier charge is calculated from your items and delivery
                    state at checkout. Self-pickup is free.
                  </span>
                </div>
            ...
            {portalReady &&
              createPortal(
                <>
                  {summaryOpen && (
                    <div
                      className="merch-cart-summary-backdrop"
                      onMouseDown={closeSummary}
                    >
                      <section
                        className="merch-cart-summary-sheet"
                        role="dialog"
                        aria-modal="true"
                        aria-labelledby="mobile-cart-summary-title"
                      ...
                      >
                        ...
                        <div className="merch-cart-shipping-note">
                          <span>
                            Courier charge is calculated from your items and
                            delivery state at checkout. Self-pickup is free.
                          </span>
                        </div>
                        <div className="merch-cart-total">
                          <span>Total</span>
                  ...
                  <div className="merch-cart-mobile-checkout">
                    <button
                      type="button"
                      className="merch-cart-summary-toggle"
        ...
        (() => {
          ...
          return (
            <CartMerchandiseEditor
              item={editingItem}
              product={product}
              reservedQuantityByVariant={items.reduce<Record<number, number>>(
                (totals, candidate) => {
                  if (
                    candidate.id !== editingItem.id &&
                    candidate.type === "merchandise" &&
                    candidate.bundleVariantId
                  ) {
                    totals[candidate.bundleVariantId] =
                      (totals[candidate.bundleVariantId] || 0) +
                      candidate.quantity;
                  }
                  return totals;
                },
