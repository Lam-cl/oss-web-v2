...
const sourcePath = path.join(root, 'src/lib/productImageColors.server.ts');
const code = ts.transpileModule(fs.readFileSync(sourcePath, 'utf8'), {
  compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2020, esModuleInterop: true },
}).outputText;
const box = { exports: {} };
vm.runInNewContext(code, { module: box, exports: box.exports, require: id => id === './dataApiClient.server' ? { dataApiEnabled: () => false } : require(id), process });
...
const adminRoute = fs.readFileSync(path.join(root, 'src/app/api/admin/[...path]/route.ts'), 'utf8');
...
const publicRoute = fs.readFileSync(path.join(root, 'src/app/api/bundle/merchandise/route.ts'), 'utf8');
assert(publicRoute.includes('hiddenOptionValues'), 'storefront payload must exclude locally hidden option values');
const drawer = fs.readFileSync(path.join(root, 'src/components/admin/ProductDrawer.tsx'), 'utf8');
