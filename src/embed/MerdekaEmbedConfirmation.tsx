...
export default function MerdekaEmbedConfirmation() {
  ...
  return <main className={styles.confirmPage}><div className={styles.confirmCard}>
    ...
    <div className={styles.confirmActions}>{(status==='error'||status==='failed')&&reference&&<button type="button" onClick={()=>{setError('');setResult(null);setRetryKey(key=>key+1);}}>Check again</button>}{status==='failed'&&<a href={merdekaPublicPageUrl()}>Try payment again</a>}{status==='success'&&<a href="https://www.tonewow.com/">Back to tone wow</a>}</div>
