...
- The supplied shipping assignments were migrated to both stable catalogue UUIDs and current Bundle IDs. Authenticated readback reports 16 eligible products and zero products needing assignment.
- SUPERLITE was republished as Bundle product `107` with one Tone Excel variant (stock 44), and BIZ SIM as product `108` with one Tone Excel variant (stock 46). Both catalogue records report `clean`; their retired products `39` and `40` are absent from the public response.
- Old Tone Plus variants `210` and `212` were permanently deleted after the order audit found no references in the 22 available staging orders. Bundle removed the variants, but returned HTTP 500 when asked to delete orphan option value `157` on already soft-deleted product `39`; the promised stop-on-refusal policy was followed, so value `159` was not mutated. These orphan labels are only on retired products and are not exposed by the storefront.
- The final reverse-proxy smoke test confirms invalid/uncertain GKash returns redirect to `https://tonewow.xifuhalim.com/payment/processing`, never the internal `localhost:3002` origin.
- A build-time feature-flag omission was detected immediately during rollout. The old build was restored while rebuilding with the live staging env; final readback confirms Devices and Merchandise are enabled and the home page contains no `Coming Soon` label.
