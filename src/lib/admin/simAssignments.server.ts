...
type StoredAssignment = SimAssignment & { pukCiphertext?:string };
type StoredOrder = { updatedAt:string; assignments:StoredAssignment[] };
type Store = { version:1; orders:Record<string,StoredOrder> };
export type SimAssignmentInput = { unitKey:string; prefixId?:string; prefix:string; serial:string; puk?:string };
...
function validateStore(value:unknown):Store {
  ...
  for (const record of Object.values(source.orders)) {
    ...
    for (const item of record.assignments) if (!item || typeof item.unitKey!=='string' || typeof item.prefix!=='string' || typeof item.serial!=='string' || (item.pukCiphertext!==undefined&&typeof item.pukCiphertext!=='string')) throw corrupt();
  }
...
function encryptPuk(value:string) { const secret=process.env.ADMIN_SESSION_SECRET;if(!secret||secret.length<32)throw new SimAssignmentValidationError('SIM assignment encryption is not configured.');if(!/^\d{8}$/.test(value))throw new SimAssignmentValidationError('SIM PUK must be exactly 8 digits.');const key=createHash('sha256').update(`tonewow-sim-assignment:${secret}`).digest(),iv=randomBytes(12),cipher=createCipheriv('aes-256-gcm',key,iv),body=Buffer.concat([cipher.update(value,'utf8'),cipher.final()]);return Buffer.concat([iv,cipher.getAuthTag(),body]).toString('base64url'); }
function normalize(units:SimUnit[],input:SimAssignmentInput[],existing:StoredAssignment[],prefixOptions?:SimPrefixOption[]):StoredAssignment[] {
...
export async function saveOrderSimAssignments(orderId:number,units:SimUnit[],input:SimAssignmentInput[],file=SIM_ASSIGNMENTS_FILE,prefixOptions?:SimPrefixOption[]) { return serialized(()=>mutateStore(file,store=>{ const existing=store.orders[String(orderId)]?.assignments||[]; const assignments=normalize(units,input,existing,prefixOptions); serialConflict(store,orderId,assignments); store.orders[String(orderId)]={updatedAt:new Date().toISOString(),assignments}; return merged(orderId,units,store); })); }
