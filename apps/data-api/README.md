...
- Cross-process specialist mutations use expiring service leases.
- Admin cookies contain only an opaque session ID; Bundle tokens are encrypted at rest with `SESSION_ENCRYPTION_KEY`.
- Media uploads require a caller-supplied SHA-256 that is verified before storage.
- Catalogue archive removes active rows in one PostgreSQL transaction while retaining an exact restore manifest and the referenced MinIO objects.
...
Run every numbered migration in order, provision private `tonewow-draft` and published `tonewow-published` MinIO buckets, then configure the variables in `.env.example`. The built-in migrator records and verifies each applied migration hash. Do not use the legacy `/opt/tonewow-shop-api` database fallback credentials.
