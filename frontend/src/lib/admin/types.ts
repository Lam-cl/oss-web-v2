...
export const matchesFulfilmentStatusFilter = (order: Order, filter: string) => {
  const orderStatus = text(order.status).toUpperCase();
  const selectedStatus = text(filter).toUpperCase() || 'ALL';
  if (selectedStatus === 'CANCELLED') return orderStatus === 'CANCELLED';
  if (!isPaymentConfirmedOrder(order)) return false;
