...
export type BillingAddress = { fullName:string; email:string; phone:string; address:string; city:string; state:string; country:string; postalCode:string };
export type CourierMetadata = { service:string; trackingNo:string; expectedDeliveryDate:string; savedAt:string };
export type PaymentReferenceMetadata = { referenceNumber:string; savedAt:string };
export type OrderMetadata = { billingAddress?:BillingAddress; courier?:CourierMetadata; paymentReference?:PaymentReferenceMetadata; updatedAt:string };
type Store = { version:1; orders:Record<string,OrderMetadata> };
export const ORDER_METADATA_FILE = path.join(process.cwd(), '.data', 'order-metadata.json');
...
export async function saveCourierMetadata(orderId:number,input:Record<string,unknown>,file=ORDER_METADATA_FILE){assertEnabled();const service=clean(input.service),trackingNo=clean(input.trackingNo),expectedDeliveryDate=clean(input.expectedDeliveryDate);if(!service||!trackingNo)throw new OrderMetadataError('Courier service and tracking number are required.');if(expectedDeliveryDate&&!/^\d{4}-\d{2}-\d{2}$/.test(expectedDeliveryDate))throw new OrderMetadataError('Expected delivery date is invalid.');return serialized(()=>mutateStore(file,store=>{const previous=store.orders[String(orderId)]||{updatedAt:''};if(previous.courier)throw new OrderMetadataError('Courier information is already saved and cannot be changed.');store.orders[String(orderId)]={...previous,courier:{service,trackingNo,expectedDeliveryDate,savedAt:new Date().toISOString()},updatedAt:new Date().toISOString()};return store.orders[String(orderId)]}))}
