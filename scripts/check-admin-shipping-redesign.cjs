...
assert.match(page, /Tetapan penghantaran/);
assert.match(page, /Perlu tindakan \(\{needsAction\.length\}\)/);
assert.match(page, /Produk sudah ditetapkan \(\{completed\.length\}\)/);
assert.match(page, /Tidak perlu penghantaran/);
...
assert.match(page, /Semak & simpan/);
assert.match(page, /Kategori produk:.*Kadar & label penghantaran:.*Keutamaan pesanan campuran:/s, 'save review must summarize each change category');
...
assert.match(page, /Peraturan pesanan campuran \(Lanjutan\)/);
assert.match(page, /aria-label=\{`Alih .* ke atas`\}/);
assert.match(page, /aria-label=\{`Padam julat/);
assert.match(page, /21 unit ke atas/);
assert.match(page, /Percuma/);
...
new Function('exports', 'module', 'require', ts.transpileModule(
  `const COURIER_GROUPS = ['shirt', 'bulky', 'small', 'flyers', 'sim'];\nconst GROUP_LABELS = { shirt: 'Baju T', bulky: 'Botol, tumbler atau bunting', small: 'Barang kecil', flyers: 'Flyers', sim: 'Kad SIM' };\nexport ${helperSource}`,
  { compilerOptions: { module: ts.ModuleKind.CommonJS, target: ts.ScriptTarget.ES2022 } },
...
assert.throws(() => mergeChangedSettings(scalarBaseline, localProduct, freshProduct), /konflik/i, 'overlapping product mapping changes must conflict');
...
assert.throws(() => mergeChangedSettings(scalarBaseline, localPriority, freshPriority), /konflik/i, 'overlapping priority changes must conflict');
...
assert.throws(() => mergeChangedSettings(scalarBaseline, conflictingLocalLabel, conflictingFreshLabel), /konflik.*Baju T/i, 'overlapping group label changes must conflict');
