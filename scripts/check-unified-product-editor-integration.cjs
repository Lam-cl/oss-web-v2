...
assert.match(source, /<UnifiedProductEditor[\s\S]*?onSave=\{async \(intent\) => \{[\s\S]*?reconcileCatalogueInventoryChanges[\s\S]*?await save\(catalogueContentIntent\(intent, product!\.model, latestInventory\?\.inventory \?\? \[\]\)\)/, 'existing catalogue edits preflight and preserve operational live inventory');
...
assert.match(source, /inventoryChanges\.length[\s\S]*?method:\s*['"]PATCH['"][\s\S]*?setPendingPhotos\(\[\]\);[\s\S]*?onSaved\(/, 'successful existing save applies explicit live stock changes before closing');
...
assert.match(source, /onSaved=\{\(message\) => \{[\s\S]*?history\.replaceState\(null, '', '\/admin\/products'\)[\s\S]*?setEditor\(undefined\)[\s\S]*?flash\(message\)[\s\S]*?void load\(\)/, 'successful existing save must normalize the URL, close the editor, preserve contextual confirmation, and refresh the list');
