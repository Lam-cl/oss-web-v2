...
import { catalogueCombinationKey, rebindPublishedCatalogueModelVariantIds } from './catalogueVariantBindings';
...
export const catalogueAdminRoute={
  ...
  async updateInventory(id:string,body:unknown,token:string){
    ...
    const changes=body.changes;if(changes.some(change=>!object(change)||!exact(change,['expectedInventory','inventory','valueKeys','variantId'])||!Array.isArray(change.valueKeys)||change.valueKeys.some(key=>typeof key!=='string'||!key)||!revision(change.variantId)||typeof change.expectedInventory!=='number'||!Number.isSafeInteger(change.expectedInventory)||change.expectedInventory<0||typeof change.inventory!=='number'||!Number.isSafeInteger(change.inventory)||change.inventory<0)||new Set(changes.map(change=>catalogueCombinationKey((change as Row).valueKeys as string[]))).size!==changes.length)invalid('Each inventory change requires one unique option combination, its expected variant ID, and nonnegative expected/new stock.');
    return withInventoryLock(id,async()=>{
      ...
      const requested=changes as Array<{valueKeys:string[];variantId:number;expectedInventory:number;inventory:number}>,rowsByTuple=new Map(before.rows.map(row=>[catalogueCombinationKey(row.valueKeys),row]));
      for(const change of requested){const row=rowsByTuple.get(catalogueCombinationKey(change.valueKeys)),variant=row&&before.providerVariants.get(row.variantId);if(!row||!variant)throw new CatalogueAdminRouteError('A product option changed after this editor was opened. Reload before saving.',409);if(variant.inventory!==change.expectedInventory&&variant.inventory!==change.inventory)throw new CatalogueAdminRouteError('Live stock changed after this editor was opened. Reload and review the latest stock before saving.',409);}
      const updates=requested.flatMap(change=>{const row=rowsByTuple.get(catalogueCombinationKey(change.valueKeys))!,variant=before.providerVariants.get(row.variantId)!;return variant.inventory===change.inventory?[]:[{id:variant.id,sku:variant.sku,price:variant.price,inventory:change.inventory}];});
      ...
      const afterRowsByTuple=new Map(after.rows.map(row=>[catalogueCombinationKey(row.valueKeys),row]));for(const change of requested){const priorRow=rowsByTuple.get(catalogueCombinationKey(change.valueKeys))!,currentRow=afterRowsByTuple.get(catalogueCombinationKey(change.valueKeys)),prior=before.providerVariants.get(priorRow.variantId)!,current=currentRow&&after.providerVariants.get(currentRow.variantId);if(!current||current.inventory!==change.inventory||current.sku!==prior.sku||current.price!==prior.price)throw new CatalogueAdminRouteError(mutationFailed?'Bundle stock update failed and could not be reconciled.':'Bundle stock readback did not match the requested change.',503);}
