...
const statusRoute = read('src/app/api/merdeka-promo/status/route.ts');
const memberRoute = read('src/app/api/merdeka-promo/member/route.ts');
const checkoutRoute = read('src/app/api/merdeka-promo/checkout/route.ts');
...
assert(!confirmation.includes('writeMerdekaPayment'), 'Gateway return data must not update payment state.');
assert(!statusRoute.includes('gatewayStatus') && !statusRoute.includes('gatewayDescription'), 'Status must not trust callback fields.');
assert(!memberRoute.includes('fetchMerdekaMember'), 'Public member lookup must remain disabled until ownership verification exists.');
assert(!checkoutRoute.includes('osspayMerdeka2026'), 'Checkout must remain disabled until authoritative backend reconciliation exists.');
