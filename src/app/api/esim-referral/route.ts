...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      const paymentRecord = statusData?.data?.[0];
      paymentResolved = statusRes.ok && statusData?.systemCode === '0' && Boolean(clean(paymentRecord?.status));
      referralCode = paymentResolved ? clean(paymentRecord?.referralCode) : '';
    } catch {
    ...
    return NextResponse.json({
      ...
      source: paymentResolved ? 'payment' : 'unavailable',
      promoter: null,
    });
  } catch (error: any) {
