...
assert.equal(adapter.cataloguePackUnitLabel(['Pack size: 20 pcs']), '20 pcs per bundle');
assert.equal(adapter.cataloguePackUnitLabel(['50 pieces']), '50 pcs per bundle', 'legacy product details remain readable');
const packed = structuredClone(payload);
packed.products[0].slug = 'renamed-flyer';
packed.products[0].details.title = 'Renamed marketing print';
packed.products[0].details.description = 'Printed material\n\nProduct details:\n- Pack size: 20 pcs';
assert.equal(adapter.adaptCatalogueStorefrontPayload(packed, fallback)[0].unitLabel, '20 pcs per bundle', 'pack label must not depend on exact title or slug');
