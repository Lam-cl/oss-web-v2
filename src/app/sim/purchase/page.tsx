...
/* ── Inline referral helpers (kept local so staging doesn't need a separate
   '@/lib/referral' module — same behaviour, no new file dependency) ── */
type ReferralPrefix = 'TWE' | 'TWP';

const normalizeReferralCode = (_prefix: ReferralPrefix, code: string) => code.trim();
const buildMemberID = (prefix: ReferralPrefix, code: string) => `${prefix}-${code}`;

const REFERRAL_CONTEXT_STORAGE_KEY = 'tw_referral_context';
...
const readReferralContext = (): ReferralContext | null => {
  try {
    const raw = localStorage.getItem(REFERRAL_CONTEXT_STORAGE_KEY);
    return raw ? (JSON.parse(raw) as ReferralContext) : null;
  } catch {
    return null;
  }
};
const saveReferralContext = (context: ReferralContext) => {
  try {
    localStorage.setItem(REFERRAL_CONTEXT_STORAGE_KEY, JSON.stringify(context));
  } catch {
    /* ignore */
  }
};
    ...
    if (result.valid) {
      if (prefix === 'TWP') {
        ...
        if (ref.referenceID) {
          ...
          setPromoterError((ref as any).error || 'Unable to generate TWP reference ID. Please try again.');
        }
      } else {
