...
assert(failedPage.includes("params.get('error')"), 'failed page must read error as well as reason');
assert(component.includes("reason === 'Payment processing error'"), 'only the exact processing error may activate polling');
